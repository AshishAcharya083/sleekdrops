...
- **Two module instances: ruled out.** The production bundle (`astro build` with the ingest key set) emits a single hoisted script for both client entry points - `BaseLayout`'s `import '../scripts/chrome'` and the `PrivacyPreferences` island - with one copy of `src/lib/analytics.ts` and one copy of the SDK, and one `<script type="module">` tag per page. Two instances also could not produce the observed shape: only the preferences island calls `boot()`, so the second instance's `Page Viewed` would be stranded in an unflushed buffer rather than duplicated.
...
- `npm run check` (`astro check`) - 0 errors, so the full event-wiring graph type-checks (the `EVENTS` map, every `data-track` call site, and the privacy-preferences island that boots the gate). `track()` now takes the taxonomy union rather than `string`, so this run is also what proves no code call site names an undeclared event.
