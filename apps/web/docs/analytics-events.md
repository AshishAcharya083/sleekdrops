...
Turning it back off stops **both** sinks: the DevTeam client is detached and shut down, and GA4 - which cannot be unloaded once its tag is in the DOM, and which emits `user_engagement` on its own - is switched off through its `window['ga-disable-<MEASUREMENT_ID>']` flag with its `_ga` / `_ga_*` cookies expired. The flag is cleared again on a re-grant, because the tag is only ever loaded once per document.
