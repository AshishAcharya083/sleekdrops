    ...
    <>
      ...
      {queue && (queue.running > 0 || queue.queued > 0) && (
        <div className="notice-banner" role="status">
          Topic discovery is {queue.running > 0 ? 'searching now' : 'waiting to start'}
          {queue.queued > 0 &&
            ` · ${queue.queued} ${queue.queued === 1 ? 'search' : 'searches'} queued`}
        </div>
      )}
        ...
        <button
          ...
          onClick={() =>
            act(() => api('/api/scout', { method: 'POST' }), 'scout', {
              ok: 'Topic search added to the queue.',
              onTracked: () => track(EVENTS.scoutRunQueued, { surface: 'topics' }),
            })
          }
        >
