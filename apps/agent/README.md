...
raise, with an optional per-stage override in `STAGE_TIMEOUT_SECONDS` next to
the stage map in `pipeline/runner.ts`. It is deliberately not an admin setting:
a timeout is a safety guard, and what an operator acts on is the outcome. A
...
platform is up rather than at the next restart. **Retry** in the article panel
re-queues it at the stage that stopped.
